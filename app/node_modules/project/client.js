...
function fillPage(data, clickEvent) {
  ...
  if (!clickEvent) {
    filtertable(data.repos, repoEvent);
  }
...
function streamReader(stream) {
  stream.on('json', function(data) {
    ...
    if (data.remove) {
      ...
      delete projectData.repos[data.remove];
    ...
    if (data.repo) {
      if (data.error) {
        return responseP.innerHTML = data.error;
      }
...
function Load(stream, match) {
  ...
  dom.on(okBtn, 'click', function() {
    ...
    if (!/\w+\/\w+/.test(repoInput.value)) {
      responseP.innerHTML = 'Need something like `name/project`';
      return;
    }
